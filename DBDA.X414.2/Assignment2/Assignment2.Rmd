...
row.names(test.data)<-c("vector 1","vector 2")
...
Calculating the PDFs using a normal distribution using the mean and variance previously calculated.
...
calculatePdfs <- function(input.data) {
  ...
  pdfs <- sapply(iris.Prob.gp.by.Species$Species, function(x) {
    ...
    pdf <- dnorm(
      ...
      mean = as.numeric(this.mean),
      sd = as.numeric(sqrt(this.var))
      )
    return(pdf)
  })
...
#pdfs.per.vector
#pdfs <- apply(test.data,2,function(x){return(x)})
#pdfs <- apply(test.data,2,calculatePdfs)

for(i in 1:nrow(test.data)){
  pdfs <- calculatePdfs(test.data[i,])
  pdfs
  #pdfs.per.vector <- c(pdfs.per.vector,pdfs)
...
Using the dnorm function in r to calculate the value in the normal PDF. It uses standard deviation so getting the square root of the variance before calling it.

The table will be read the value of the PDF of “row name” given “column name” for instance p(Sepal.Length|setosa)=X 

Multiplying the results of all the PDF values by the class probability too and picking up the class of the highest number.

```{r iris_class}
#posteriors.per.vector <- list()

#for (i in 1:length(pdfs.per.vector)) {

#i = 1
#probs <- rbind(pdfs.per.vector[[i]],iris.Prob.gp.by.Species$Prob)

#i =1
#input.pdfs <- pdfs.per.vector[[i]]
...
#posteriors.per.vector[[i]] <- posteriors %>% mutate(winner=winner)

 
#}
#i=1
row.names(posteriors.per.vector)<-c("vector 1","vector 2")
knitr::kable(posteriors.per.vector)
